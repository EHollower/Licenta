"""
Reformat LOJ “#127. 最大流 加强版” test cases (1-indexed) into 0-indexed format.
Download from: https://loj.ac/p/127/files (login required).
...
ZIP_FILE = Path(__file__).parents[2] / 'related_problems' / '9_loj_127.zip'
...
for idx, in_path in enumerate(TMP_DIR.rglob('*.in')):
    ...
    out_file = OUT_DIR / f"loj127-{idx}.in"
    out_file.write_text('\n'.join([header] + edges) + '\n')
